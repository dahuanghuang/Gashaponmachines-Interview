...
class MallRecordViewModel: PaginationViewModel<MallProductExchangeRecordsEnvelope.Record> {
    ...
    override init() {
        ...
        let response = request
            .flatMapLatest { page in
                AppEnvironment.current.apiService.getMallProductExchangeRecords(page: page).materialize()
            }
            .share(replay: 1)
