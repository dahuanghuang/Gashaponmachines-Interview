class YiFanShangLivestreamRewardView: UIView {

    let icon = UIImageView.with(imageName: "yfs_reward_logo")

    init(eggInfos: [YiFanShangLivestreamInfo.LiveEggInfo]) {
        super.init(frame: .zero)
        
        let count = eggInfos.count
        ...
        for idx in 0..<count {
            ...
            eggIcon.gas_setImageWithURL(eggInfo.eggIcon)
            ...
            if let tCount = eggInfo.totalEggCount {
                totalLb.text = "/\(tCount)"
            }
            ...
            countLb.text = eggInfo.eggContent
