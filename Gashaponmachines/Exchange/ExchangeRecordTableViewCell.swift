...
final class ExchangeRecordTableViewCell: BaseTableViewCell {

    var increase: String? {
        didSet {
            self.valueLabel.textColor = .new_orange
            self.valueLabel.text = "+\(increase ?? "0")蛋壳"
        }
    }
    ...
    func configureWith(record: EggPointLogEnvelope.EggPointLog, isFirst: Bool, isLast: Bool) {
